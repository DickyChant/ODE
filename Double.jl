...
function energy(theta1,theta2,dtheta1,dtheta2)
    ret = (m1*l1^2/6+m2*l1^2/2)*dtheta1^2+m2*l2^2/6*dtheta2^2+m2*l1*l2*dtheta1*dtheta2/2-(m1/2+m2)*g*l1*cos(theta1)-m2/2*g*l2*cos(theta2)
    return ret
end
...
plot(ts,[theta1e,theta2e],title="Double pendulum Euler, θ1 & θ2",label = ["θ1" "θ2"])
savefig("dpe.pdf")
plot(ts,[dtheta1e,dtheta2e],title="Double pendulum Euler, θ1_dot & θ2_dot",label = ["θ1_dot" "θ2_dot"])
savefig("dped.pdf")
...
for i in 1:2500
    ...
    halfd1 = tmpd1 + dt*ddtheta1(tmp1,tmp2)
    halfd2 = tmpd2 + dt*ddtheta2(tmp1,tmp2)
    new1 = tmp1+2*dt*halfd1
    new2 = tmp2+2*dt*halfd2
    push!(theta1f,new1)
    push!(theta2f,new2)
    push!(dtheta1f,halfd1+ddtheta1(new1,new2)*dt)
    push!(dtheta2f,halfd2+ddtheta2(new1,new2)*dt)
end
